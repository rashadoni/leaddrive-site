...
import { CAPS, CapArt } from './AgentCaps';
import type { Lang } from '@/lib/product-map';
import { TypedText, useTypewriter } from './Typewriter';

const COPY = {
  ru: { eyebrow: 'AI-агент в вашей CRM', h1: 'AI-агент', h2: 'LeadDrive.', sub: 'Скажите, что нужно сделать. Агент соберёт сценарий из модулей CRM и покажет его на проверку.', prompt: 'Оцени новые лиды из WhatsApp, назначь горячие менеджеру и подготовь резюме к встрече', agent: 'AI-агент LeadDrive', result: 'Что умеет агент в вашей CRM:', steps: [['Оценка лидов', 'Inbox → лид'], ['Горячий лид → менеджер', 'эскалация'], ['Черновик ответа', 'WhatsApp'], ['Задача: встреча', 'календарь'], ['Резюме после звонка', 'Da Vinci']], review: 'Режим «проверка»: ничего не отправляется без подтверждения сотрудника.', cta: 'Показать на демо' },
  az: { eyebrow: 'CRM-inizdə AI agent', h1: 'LeadDrive', h2: 'AI agenti.', sub: 'Nə etmək lazım olduğunu deyin. Agent CRM modullarından ssenari yığıb yoxlamağa göstərəcək.', prompt: 'WhatsApp-dan gələn yeni lidləri yoxla, ən perspektivlilərini menecerə ötür və görüş üçün qısa arayış hazırla', agent: 'LeadDrive AI agenti', result: 'Agent CRM-inizdə nə bacarır:', steps: [['Lid qiymətləndirmə', 'Inbox → lid'], ['Perspektivli lid → menecer', 'eskalasiya'], ['Cavab qaralaması', 'WhatsApp'], ['Tapşırıq: görüş', 'təqvim'], ['Zəngdən sonra xülasə', 'Da Vinci']], review: '«Yoxla» rejimi: əməkdaşın təsdiqi olmadan heç nə göndərilmir.', cta: 'Demoda göstərək' },
  en: { eyebrow: 'An AI agent in your CRM', h1: 'LeadDrive', h2: 'AI agent.', sub: 'Say what needs to be done. The agent assembles a scenario from CRM modules and shows it for review.', prompt: 'Score new WhatsApp leads, assign the hot ones to a manager and prepare a meeting summary', agent: 'LeadDrive AI agent', result: 'What the agent can do in your CRM:', steps: [['Lead scoring', 'inbox → lead'], ['Hot lead → manager', 'escalation'], ['Reply draft', 'WhatsApp'], ['Task: meeting', 'calendar'], ['Post-call summary', 'Da Vinci']], review: 'Review mode: nothing is sent without an employee confirming it.', cta: 'Show on a demo' },
} as const;
      ...
      <div className="wrap ai-presenter-grid">
        <div className="ai-presenter-copy">
          ...
          <div className="ai-agent-reply">
            ...
            <ol className="ai-caps">
              {CAPS[lang].map((cp, i) => (<li key={cp.kind} style={{ transitionDelay: `${i * 160}ms` }}><CapArt kind={cp.kind} /><b>{cp.title}</b><small>{cp.sub}</small></li>))}
            </ol>
